...
export default function AdminEditTemplatePage() {
  const { templateId: idFromUrl } = useParams(); // Ambil ID dari URL
  ...
  // Efek untuk memuat data template berdasarkan ID dari URL
  useEffect(() => {
    if (idFromUrl && idFromUrl !== 'new') {
      // Panggil fungsi load dari hook editor untuk mengambil data lengkap
      editor.loadTemplate(idFromUrl);
    }
  }, [idFromUrl]); // Jalankan hanya saat ID dari URL berubah
