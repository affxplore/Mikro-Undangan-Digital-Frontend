...
export default function TemaPage() {
  ...
  const filtered = useMemo(() => {
    let data = dummyTemplates.filter((t) => {
      ...
      if (!q) return true;
      return (t.name + " " + t.tagline + " " + t.category)
        .toLowerCase()
        .includes(q);
    });
          ...
          <div className="mx-auto max-w-7xl">
            ...
            >
              ...
              <p className="text-lg text-blue-100/95 max-w-2xl mx-auto">
                Temukan template undangan digital yang sempurna. Desain
                profesional, mudah dikustomisasi, dan siap pakai.
              </p>
            </motion.div>
            ...
            >
              ...
              <div className="flex items-center gap-2 bg-white/15 backdrop-blur-sm rounded-2xl px-3 py-2 text-white">
                ...
                <label htmlFor="sort" className="text-sm opacity-90">
                  Urutkan
                </label>
                <select
                  id="sort"
                  ...
                  className="bg-transparent text-white/95 font-medium focus:outline-none"
                >
          ...
          <section className="lg:col-span-3">
                ...
                <div className="hidden sm:flex gap-1 rounded-xl bg-slate-100 p-1">
                  <button
                    ...
                    className={`p-2 rounded-lg ${
                      viewMode === "grid" ? "bg-white shadow" : "text-slate-600"
                    }`}
            ...
            {filtered.length === 0 && (
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="p-12 text-center bg-white/70 rounded-3xl border border-white/50"
              >
        ...
        {selectedTemplate && (
          ...
          >
            ...
            <motion.div
              ...
              animate={{ scale: 1, y: 0 }}
              exit={{ scale: 0.9, y: 20 }}
              className="relative bg-white rounded-3xl shadow-2xl w-full max-w-6xl max-h-[90vh] overflow-hidden z-50 border border-white/20"
            >
              {/* Header */}
                    ...
                    <div className="bg-slate-50 rounded-2xl p-4 sm:p-8 min-h-[500px] flex items-center justify-center">
                      ...
                      {previewMode === "mobile" ? (
                        <motion.div
                          initial={{ scale: 0.95 }}
                          animate={{ scale: 1 }}
                          className="w-[320px] h-[640px] bg-white rounded-[3rem] shadow-2xl overflow-hidden border-8 border-slate-800"
                        >
                          <iframe
                        ...
                        <motion.div
                          initial={{ scale: 0.95 }}
                          animate={{ scale: 1 }}
                          className="w-full h-96 bg-white rounded-2xl shadow-xl overflow-hidden border"
                        >
                          <iframe
                            src={selectedTemplate.previewUrl}
                            title={selectedTemplate.name}
                  ...
                  <aside className="lg:col-span-1 space-y-6">
                    <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl p-6 border border-blue-100">
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <span className="font-semibold">
                            {selectedTemplate.views.toLocaleString()}
                          </span>
                        </div>
                        <div className="flex justify-between items-center">
                    ...
                    <div className="space-y-3">
                      <motion.button
                        whileHover={{ scale: 1.02 }}
                        whileTap={{ scale: 0.98 }}
                        onClick={() => chooseTemplate(selectedTemplate)}
                        className="w-full px-6 py-4 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white font-bold rounded-2xl flex items-center justify-center gap-3 transition"
                      >
                      ...
                      <motion.button
                        whileHover={{ scale: 1.02 }}
                        whileTap={{ scale: 0.98 }}
                        className="w-full px-6 py-4 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold rounded-2xl flex items-center justify-center gap-2 transition"
                      >
                      ...
                      <motion.button
                        whileHover={{ scale: 1.02 }}
                        whileTap={{ scale: 0.98 }}
                        className="w-full px-6 py-4 border-2 border-slate-200 hover:border-blue-300 text-slate-700 font-semibold rounded-2xl flex items-center justify-center gap-2 transition"
                      >
...
function TemplateCard({ t, index, onPreview, onChoose }) {
  return (
    <motion.div
      ...
      className="group bg-white/80 backdrop-blur rounded-3xl shadow-lg hover:shadow-2xl border border-white/40 overflow-hidden transition"
    >
      {/* Preview */}
      ...
      <div className="p-6">
        ...
        <div className="flex gap-3">
          {/* <motion.button
            ...
            whileTap={{ scale: 0.97 }}
            onClick={onPreview}
            className="flex-1 px-4 py-3 rounded-2xl border-2 border-slate-200 hover:border-blue-300 text-slate-600 hover:text-blue-600 font-medium transition flex items-center justify-center gap-2"
          >
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.97 }}
            onClick={onPreview}
            className="flex-1 px-4 py-3 rounded-2xl bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white font-semibold transition flex items-center justify-center gap-2"
          >
...
function TemplateRow({ t, index, onPreview, onChoose }) {
  return (
    <motion.div
      ...
      className="bg-white/80 backdrop-blur rounded-2xl border border-white/50 shadow hover:shadow-lg p-4 flex items-center gap-4"
    >
      <div
        className={`w-36 h-24 rounded-xl ${t.thumbAccent} relative overflow-hidden grid place-items-center`}
      >
        ...
        <motion.div
          ...
          className="w-16 h-10 rounded-xl shadow-xl"
          style={{
      ...
      <div className="flex items-center gap-2">
        {/* <motion.button
          ...
          whileTap={{ scale: 0.97 }}
          onClick={onPreview}
          className="px-3 py-2 rounded-xl border border-slate-200 hover:border-blue-300 text-slate-700"
        >
          Demo
        </motion.button> */}
        <motion.button
          ...
          whileTap={{ scale: 0.97 }}
          onClick={onPreview}
          className="px-3 py-2 rounded-xl bg-gradient-to-r from-blue-600 to-purple-600 text-white"
        >
          Pilih
        </motion.button>
      </div>
