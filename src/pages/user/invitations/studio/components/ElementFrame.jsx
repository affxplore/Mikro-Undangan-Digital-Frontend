...
function useDraggableRect(el, boundsRef, enabled = true, onCommit, snapEnabled = true, snapSize = 5) {
  ...
  }, [pos, boundsRef, enabled, el.w, el.h, onCommit, snapEnabled, snapSize]);

  return { ref, pos, dragging: draggingRef.current };
}

function useResizable(el, boundsRef, enabled = true, onCommit, snapEnabled = true, snapSize = 5) {
...
export default function ElementFrame({ el, selected, boundsRef, onSelect, onDragCommit, onResizeCommit, onInlineChange, snapEnabled = true, snapSize = 5 }) {
  ...
  const { size, start } = useResizable({ ...el, x: pos.x, y: pos.y }, boundsRef, true, (rect) => onResizeCommit(rect), snapEnabled, snapSize);
